...
// Router registers routes against their dedicated handlers and
// dispatches those handlers. It implements the http.Handler
// interface, so it is registered to serve requests:
//
//	var router = sonate.NewRouter()
//
//	func main() {
//		http.Handle("/", router)
//	}
//
//	This will send all incoming requests to the provided router.
type Router struct {
	// NotFoundHandler is the configurable handler for routes that
	// do not match any path(s). This can be used to make your own
	// 404 responses.
	NotFoundHandler http.Handler

	// MethodNotAllowedHandler is the configurable handler for requests
	// in which the request method does not match the route. This can be
	// used to make your own 405 responses.
	MethodNotAllowedHandler http.Handler

	// Routes to be matched.
	routes []*Route

	// Routes by name. This is used for URL building.
	namedRoutes map[string]*Route

	// Slice of middlewares to be called after any match is found.
	middlewares []middleware

	// Config. Shared with `Route`
	routeConf
}
...
// Route configurations shared between `Router` and `Route`.
...
//
// If the request matches a route of this router or one of its sub-routers the Route,
// Handler, and Vars fields of the match argument are filled and this function
// returns true.
//
// If the request does not match any of this router's or its sub-routers' routes
...
func (r *Router) Match(req *http.Request, match *RouteMatch) bool {
	for _, route := range r.routes {
		if route.Match(req, match) {
			// Build the required middleware chain.
